...
/////////////////////after resend verify not working
...
//     if (existing) {
//       // If account exists but never verified — delete it and let them re-register
//       // This handles: typo in email, never got the email, want to change password before verifying
//       if (!existing.isEmailVerified) {
//         await User.deleteOne({ _id: existing._id });
//         await EmailVerification.deleteMany({ userId: existing._id });
//       } else {
...
export const verifyEmail = async (req, res, next) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(record.userId, { isEmailVerified: true, $unset: { unverifiedAt: "" } }, { new: true }).populate("currentCompanyId");
    await EmailVerification.deleteOne({ _id: record._id });
