      ...
      <div className="mb-6">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h1 className="font-display text-xl sm:text-2xl font-bold text-ink">Referrer Dashboard</h1>
            <p className="text-ink-muted text-xs sm:text-sm mt-1 truncate">Managing referrals for {companyName}</p>
          </div>
          {profile && (
            <button
              onClick={handleToggle}
              disabled={toggling}
              className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all border ${
                profile.isActive
                  ? "bg-emerald-soft border-emerald-200 text-emerald-700"
                  : "bg-rose-50 border-rose-200 text-rose-600"
              }`}
            >
              <span className={`w-2 h-2 rounded-full flex-shrink-0 ${profile.isActive ? "bg-emerald-500" : "bg-rose-400"}`} />
              {toggling ? "..." : profile.isActive ? "Available" : "Unavailable"}
            </button>
          )}
        </div>
