...
export default function NotificationBell() {
  const [open, setOpen] = useState(false);
  const { data } = useGetNotificationsQuery(undefined, { pollingInterval: 30000, skipPollingIfUnfocused: true });
  const [markAllRead] = useMarkAllReadMutation();
  const [markRead]    = useMarkReadMutation();
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-30" onClick={handleClose} />
          <div className="fixed sm:absolute right-2 sm:right-0 top-16 sm:top-11 z-40 w-[calc(100vw-16px)] sm:w-80 bg-white rounded-2xl shadow-2xl border border-default overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-default">
              <p className="font-semibold text-ink text-sm">Notifications</p>
              {unread > 0 && (
                <button onClick={handleMarkAll} className="text-xs text-accent hover:underline font-medium">Mark all read</button>
